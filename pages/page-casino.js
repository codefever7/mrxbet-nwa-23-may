...
import ModelMultiple from '../src/components/Jackpots/ModelMultiple'
import ListItem from '../src/components/Winners/ListItem'
...
class index extends Component {
  ...
  getLobbies = (role, slug, category) => {
    ...
    WPService.getLobbies(this.props.lang, role, slug, category).then((res) => {
      if (isArray(res)) {
        ...
        const set = { messageTitle: locale.t('informations'), messageDesc: res.Status, messageDetail: '', messageType: 'error' }
  ...
  render() {
    ...
    const { isMobile, loading, isConnected, lobbiesData,loadGame } = this.state
    let lobbiesTypeName = ''
    if (page === 'casino' || page === 'live-casino') {
      lobbiesTypeName = 'custom'
    }
    return (
      <Fragment>
        ...
        <Layout {...this.props}>
          {
            isConnected && lobbiesData && loadGame && (
            <section className={`PageCasino ${page}`}>
              <LayoutGame {...this.props} lobbiesData={lobbiesData} lobbiesTypeName={lobbiesTypeName} />
            </section>)
          }
          {/* <section className="jackpots-multiple">
            {isConnected && !isNull(isLogin) ? <ModelMultiple isMobile={isMobile} {...this.props} /> : null}
          </section>
          <section className="winners-list">
            {isConnected && !isNull(isLogin) ? <ListItem isMobile={isMobile} {...this.props} /> : null}
          </section> */}
        </Layout>
      </Fragment>
    )
