...
class GamesBonus extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            gamesSecond: [],
            order: 'desc',
            orderByField: 'bonusContribution',
            totalPageCount: 0
        }
    }
    ...
    getGames = (pageIndex, pageSize) => {
        ...
        params.expectedFields = casinoParams.FIELDS.BonusContribution + casinoParams.FIELDS.Slug + casinoParams.FIELDS.Vendor + casinoParams.FIELDS.Name
        ...
        params.sortFields = [
            {
                "field": casinoParams.FIELDS.BonusContribution,
                "order": "DESC"
            }
        ]
        ...
        let boxFirst = !isUndefined(dataFirst) && dataFirst.length ? dataFirst.map((item, index) => {
            return (
                <tr key={index}>
                    ...
                    <td className="last">{parseFloat(item.bonusContribution * 100).toFixed(2)}%</td>
                </tr>
            )
        }) : ''
                        ...
                        <Table className="table-bordered fixed_header">
                            <thead className="fixedHeader">
                                <tr>
                                    ...
                                    <th className={`last ${(orderByField == 'bonusContribution' ? `active ${order == 'desc' ? 'asc' : 'desc'}` : '')}`} onClick={() => this.filterChange('bonusContribution')}>{locale.t('bonusContribution')}</th>
                                </tr>
                            </thead>
                            {
                                boxFirst != '' ? <tbody>
