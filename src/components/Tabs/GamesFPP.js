...
class GamesFPP extends Component {
    ...
    getGames = (pageIndex, pageSize) => {
        ...
        params.expectedFields = casinoParams.FIELDS.FPP + casinoParams.FIELDS.Slug + casinoParams.FIELDS.Vendor + casinoParams.FIELDS.Name
        ...
        let boxFirst = !isUndefined(dataFirst) && dataFirst.length ? dataFirst.map((item, index) => {
            return (
                <tr key={index}>
                    ...
                    <td className="last">{parseFloat(item.fpp * 100).toFixed(2)}%</td>
                </tr>
            )
        }) : ''
                        ...
                        <Table className="table-bordered fixed_header">
                            <thead className="fixedHeader">
                                <tr>
                                    ...
                                    <th className={`last ${(orderByField == 'fpp' ? `active ${order == 'desc' ? 'asc' : 'desc'}` : '')}`} onClick={() => this.filterChange('fpp')}>{locale.t('rates')}</th>
                                </tr>
                            </thead>
                            {
                                boxFirst != '' ? <tbody>
