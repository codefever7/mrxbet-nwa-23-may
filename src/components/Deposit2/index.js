...
export class Deposit extends PureComponent {
                    ...
                    paymentForm.submitCvv({
                        ...
                        (res) => {
                            if (res.Success) {
                                ...
                                //const set = { messageTitle: locale.t('error'), messageDesc: locale.t('mrXBet'), messageDetail: locale.t('canNotDepositContactSupport'), messageType: 'error' }
                                if (this.loadBlock) this.loadBlock.isOpen(false)
                                //this.props.onSetMessageModal(set)
                            }
                        },
    ...
    getPaymentMethodLink = (method, image, type = null, sub = null) => {
        ...
        return <PaymentMethod
            ...
            transactionLimit={`${getSymbol(currency)}20 - ${getSymbol(currency)}1000`}
            redirectURL={link}
                    ...
                    <Fragment key={0}>
                        ...
                        <div className="payment-list-row">
                            ...
                            <PaymentMethod
                                ...
                                processingType={locale.t('Instant')}
                                transactionLimit={`${getSymbol(userInfo.currency || "€")}10 - ${getSymbol(userInfo.currency || "€")}1000`}
                                getPaymentMethodCfg={this.handleSelectBank}
                                payment={res}
                                imagePath={`/static/images/Paymentlogo/visa-ms.png`}
                                type='row'
                            />
                            {/* {this.getPaymentMethodLink('ips', 'visa-ms.png')} */}
    ...
    _visaVIP = (res) => {
        ...
        if (isUndefined(userID)) { 
            //const set = { messageTitle: locale.t('textNotLoggedIn'), messageDesc: locale.t('mrXBet'), messageDetail: "", messageType: 'error' };
            //this.props.onSetMessageModal(set);
            return null;
        }
    ...
    _crypto = (paymentMethods) => {
        const { session: { userInfo: { currency = "€" } } } = this.props
        let bankRender = []
        if (paymentMethods.find(x => x.code == 'MoneyMatrix_CoinsPaid')) {
            ...
            let html = (
                <PaymentMethod
                    methodName={'crypto'}
                    processingType={locale.t('Instant')}
                    transactionLimit={`${getSymbol(currency)}30 - ${getSymbol(currency)}5000`}
                    getPaymentMethodCfg={this.handleSelectBank}
                    payment={res}
            ...
            forEach(paymentMethods, (res) => {
                if (res.code != 'MoneyMatrix_CoinsPaid'
                    ...
                    let html = (
                        ...
                        methodName={res.name}
                        processingType={locale.t('Instant')}
                        transactionLimit={`${getSymbol(currency)}1 - ${getSymbol(currency)}150`}
                        getPaymentMethodCfg={this.handleSelectBank}
                        payment={res}
                        imagePath={imgPath}
                        type='row'
